...
// Das Spiel auf der Konsole, rundenbasiert. Lag frueher komplett in
// Main.java, ist jetzt eine eigene Klasse neben SwingGame - beide spielen
// dasselbe Spiel, nur mit anderer Oberflaeche.
public class ConsoleGame {
    private static final int MAP_WIDTH = 13;
    private static final int MAP_HEIGHT = 11;

    public static void start(Path mapFile) {
        ...
        GameMap map = loadMap(view, mapFile);
        if (map == null) {
            view.close();
            return;
        }
    ...
    private static GameMap loadMap(ConsoleView view, Path mapFile) {
        if (mapFile == null) {
        ...
        try {
            GameMap map = new MapRepository().load(mapFile);
            view.showMessage("Map geladen aus " + mapFile);
            return map;
